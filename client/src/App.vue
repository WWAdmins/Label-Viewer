...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            // If supplier: sets the list of options for the bottle types
            // If stock code: creats list of all bottles with stock codes, makes formatted display string, 
            //         fills stockCodeKey to allow retrevial of specific bottle specs from a stock code string
            // Clears all dependent fields
            // supplierSelection: selected option from the multi select (v-bind updates slower so this is needed to generate bottleTypes list)
            ...
            // When a stock code is selected:
            // Set bottleID, bottleType and bottleSpec
            // Calls spec prep
            // stockCodeSelection: selected stock option (v-bind is slow so this is used to fetch required data based on stock code)
            ...
            // When the bottle is selected, sets the bottle spec to the spec for that specific bottle then calls specPrep
            // bottleIdSelection: selected bottle id (v-bind is slow so this is used to set bottle spec)
            ...
            // Dissables the back label and second front label if front label max width would define it as a wrap around
            checkWrapAround() {
            ...
            // Sorts the given list into the irder given in the targte list and returns the list in its sorted form
            ...
            // Updates validLabelOptions (calls targetSort on this using labelSortTemplate fro constants)
            // Adds label/medal to active labels/medals
            // Updates globalPositions.latest.id to "global" to force updates in child measurement forms
            ...
            // array: array for the item to be removed from
            // item: item that is to be removed from the array
            // returns: array with item is removed
            ...
            // When a label is deslected in the multiselect
            // Removes dependent labels from valid options
            // Calls cleanLabel
            // Calls updateMaxWidth
            // Calls clearPreview and updatePreview to update label previews
            // Calls checkGlobalInvalid
            ...
            // Updates the maxWidth information for the given side
            // Calls checkWrapAround
            ...
            // Resets warned (allows orange zone warning message to appear again)
            // Directly calls clearForm on measurementForm componets through refs 
            //        - Refs call is used as this is an event and it is not sutable to change the child's state
            // Resets globalPositions and labelStatuses, but maintains active labels and medals lists
            // Sets medal mode to false (switches back to label mode if in medal mode)
            // Calls checkWrapAround and checkGlobalInvalid to clear and reset any errors
            // Calls clearPreview to clear label previews
            clearForm() {
                ...
                // Mantain lists of which labels and medals are active
            ...
            // returns: globalInvalid statu as a boolean
            checkGlobalInvalid() {
                ...
                var medal = Object.keys(this.globalPositions.medal).every((element)=> {
                    if (this.globalPositions.medal[element] != null) {
                        return this.globalPositions.medal[element].valid;
                    } else {
                        return true;
                    }
                });
                medal |= !this.labelStatuses.medal.enabled;

                this.showInvalid = !(front && back && medal);
            },
            ...
            // Calls updateMaxWidth
            // Calls checkGlobalInvalid
            // Calls updatePreview if the emitting label sent measurements
            // Shows orange zone popup warning if not yet shown or a form has been reset
            // payload: json containing information for the emit
            //  - constains:
            //      warning: type of warning
            //      labelId: id of emitting label
            //      side: side of emitting label
            //      type: source of cause of emit (ie. input id, props, etc)
            //      form: measurents of emitting label
            ...
            // Updates globalPositions
            // Updates labelStatuses.filled (a label is only filled if it is valid)
            // Calls checkGlobalInvalid
            // Calls updatePreview
            // payload: json containing information for the emit
            //  - constains:
            ...
            // Updates globalPositions
            // Updates labelStatuses.filled if not a medal and all measurements are filled
            // Calls updateMaxWidth if not medal
            // Calls checkGlobalInvalid
            // Calls updatePreview if label has measurements
            // payload: json containing information for the emit
            //  - constains:
            //      labelId: id of emitting label
            //      side: side of emitting label
            ...
            // Claculates the adjusted width of the label (adjused to 2d distance over bottle curve)
            // Calls displayLabel
            // side: side of the label {'front', 'back', 'medal'}
            // main: the list of label measurements of labels on the given side
            ...
            // Claculates the adjusted width of the labels overflow (adjused to 2d distance over bottle curve)
            // Calls displayLabel
            // side: side of the label {'front', 'back', 'medal'}
            // overflow: the list of label measurements of labels that overflow from another side to this side
            //      - Overflowing is defined as the label wrapping around more than half of the total circumference 
            //          and therefore being visable on the edge of the bottle when viewed from the reverse side
            ...
            // Calls fetchDisplayMeasurements for each label and medal M1 (only M2 is never displayed)
            // If M1 exists: sets border-radius style for medal based on medal type
            // Calls displayMain and displayOverflow for front and back  labels and displayMain for M1
            updatePreview() {
            ...
            // Fetches height and height offset for the specified label and calculates theta as the width of the label as an angle on the radius of the bottle
            // If a measurement is missing is is defaulted to 0
            // If the height offset is missing, height and theta are set to 0 to hide the label
            // side: the side of the bottle this label is from
            // labelId: the id of the given label
            // returns: json {heightOffset, height, theta}
            ...
            // Clears all labels in the label preview
            clearPreview() {
            ...
            // Sets demensions of the label preview on the bottle as relative % sizes based on size of the displayed bottle
            // Places label by setting the left hand side relative to half the width of the label from the center
            // element: the id of the html element to be warpped and moved as the label preview
            // label: the measurements of the label to be displayed
